param(
  [string]$SpecPath = "swagger.json",
  [string]$TemplatesPath = "templates",
  [string]$OutDir = "..\restaurant",
  ...
  [string]$WrappersEndpoint = "http://localhost:8080/api/internal/response-wrapper-catalog",
  [string]$WrappersFile = "response-wrappers-restaurant.json",
...
if ($Group -and $Group -ne "") {
  ...
  if (-not $baseDir) { $baseDir = "." }  # Se non c'è directory parent, usa la directory corrente
...
# Custom generator JAR - percorso diretto
$customJar = "target\custom-dart-codegen-1.0.0-SNAPSHOT.jar"
if (Test-Path $customJar) {
  Write-Host "[OK] Custom generator trovato: $customJar" -ForegroundColor Green
} else {
  Write-Host "[INFO] Custom generator non trovato, lo costruisco..." -ForegroundColor Yellow
  mvn -q -DskipTests package 2>$null
  if (Test-Path $customJar) {
    Write-Host "[OK] Custom generator costruito: $customJar" -ForegroundColor Green
  } else {
    Write-Error "[ERROR] Impossibile costruire il custom generator JAR"
    exit 1
  }
}
...
# Esegui generator con ignore override usando il custom generator
Write-Host "[INFO] Uso custom generator Java locale..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
  ...
  Write-Error "[ERROR] Custom generator fallito! Exit code: $LASTEXITCODE"
  Write-Error "Il custom generator è obbligatorio per evitare problemi con ResponseWrapper"
  exit 1
}
...
# Pulizia import malformati generati dal custom generator
Write-Host "[INFO] Pulizia import malformati generati dal custom generator..." -ForegroundColor Yellow
$dartFiles = Get-ChildItem -Path "$OutDir\lib" -Include "*.dart" -Recurse
foreach ($file in $dartFiles) {
  $content = Get-Content -LiteralPath $file.FullName -Raw
  $originalContent = $content
  
  # Correggi import malformati con percorsi relativi errati
  $content = $content -replace "import\s+'\.\.\/\.\.\/\.\.\/restaurant_app\/templates\/package:openapi\/", "import 'package:openapi/"
  ...
  if (-not $alreadyInDeps) {
    ...
    if ($content -match "(?m)^dependencies:\s*$") {
      $content = [Regex]::Replace(
        $content,
        "(?m)^dependencies:\s*$",
        "dependencies:`n  common_lib:`n    path: ../"
      )
    } else {
      ...
      $content = [Regex]::Replace(
        ...
        { param($m) "${($m.Groups[1].Value).TrimEnd()}`n  common_lib:`n    path: ../`n${$m.Groups[2].Value}" },
        1
      )
      if (-not ($content -match "(?ms)^dependencies:\s*[\s\S]*?\n\s*common_lib:")) {
        # Se non ha funzionato, appendi una sezione dependencies minimale
        $content += "`ndependencies:`n  common_lib:`n    path: ../`n"
      }
    }
