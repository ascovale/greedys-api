...
public interface CustomerNotificationDAO extends JpaRepository<CustomerNotification, Long> {
     ...
     */
    @Query("SELECT c FROM CustomerNotification c WHERE c.userId = :userId ORDER BY c.creationTime DESC")
    List<CustomerNotification> findByUserId(@Param("userId") Long userId);

    /**
     * Trova tutte le notifiche NON LETTE di un customer.
     */
    @Query("SELECT c FROM CustomerNotification c WHERE c.userId = :userId AND c.read = false ORDER BY c.creationTime DESC")
    List<CustomerNotification> findUnreadByUserId(@Param("userId") Long userId);

    /**
     * Conta le notifiche NON LETTE di un customer.
     */
    @Query("SELECT COUNT(c) FROM CustomerNotification c WHERE c.userId = :userId AND c.read = false")
    ...
    @Query("UPDATE CustomerNotification c SET c.read = true, c.readAt = :readAt WHERE c.id = :notificationId")
     ...
     */
    @Modifying(clearAutomatically = true)
    @Query("UPDATE CustomerNotification c SET c.read = true, c.readAt = :readAt WHERE c.userId = :userId AND c.read = false")
     ...
     */
    @Modifying(clearAutomatically = true)
    @Query("UPDATE CustomerNotification c SET c.read = true, c.readByUserId = :readByUserId, c.readAt = :readAt " +
