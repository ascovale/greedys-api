...
/**
 * Customer Challenge Controller
 * Handles challenge operations for customers (view challenges, leaderboards)
 */
@RestController
@RequestMapping("/customer/challenges")
...
public class CustomerChallengeController {
    ...
    /**
     * Get active challenges available to customers
     */
    @GetMapping
    public ResponseEntity<Page<Challenge>> getActiveChallenges(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        log.debug("Fetching active challenges for customers");
    ...
    public ResponseEntity<List<Challenge>> getFeaturedChallenges() {
        log.debug("Fetching featured challenges");
        List<Challenge> challenges = challengeService.findFeatured(
    ...
    public ResponseEntity<Page<Challenge>> searchChallenges(
            ...
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        log.debug("Searching challenges with query: {}", query);
        Page<Challenge> challenges = challengeService.search(
                query,
                List.of(ChallengeStatus.ACTIVE, ChallengeStatus.VOTING),
    ...
    /**
     * Get challenge leaderboard (participants ordered by score)
     */
